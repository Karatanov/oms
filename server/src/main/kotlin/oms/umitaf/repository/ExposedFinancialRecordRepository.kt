...
class ExposedFinancialRecordRepository : FinancialRecordRepository {
    ...
    override fun create(projectId: Long, type: FinancialRecordType, reference: String, amount: Double, currency: String, recordDate: String, paymentDate: String?, description: String?, milestone: String?, paymentPurpose: String, eurExchangeRate: Double, eurExchangeDate: String, amountEurCents: Long, createdBy: Long) = transaction {
        ...
        val id = FinancialRecordTable.insertAndGetId {
            ...
            it[this.amount] = java.math.BigDecimal.valueOf(amount); it[this.currency] = currency; it[this.recordDate] = LocalDate.parse(recordDate); it[this.paymentDate] = paymentDate?.let(LocalDate::parse)
    ...
    override fun update(projectId: Long, uuid: String, type: FinancialRecordType, reference: String, amount: Double, currency: String, recordDate: String, paymentDate: String?, description: String?, milestone: String?, paymentPurpose: String, eurExchangeRate: Double, eurExchangeDate: String, amountEurCents: Long) = transaction {
        val n = FinancialRecordTable.update({ (FinancialRecordTable.projectId eq projectId) and (FinancialRecordTable.uuid eq uuid) }) {
            it[recordType] = type.name.lowercase(); it[referenceNumber] = reference; it[this.amount] = java.math.BigDecimal.valueOf(amount); it[this.currency] = currency; it[this.recordDate] = LocalDate.parse(recordDate); it[this.paymentDate] = paymentDate?.let(LocalDate::parse); it[this.description] = description; it[this.milestone] = milestone; it[this.paymentPurpose] = paymentPurpose; it[this.eurExchangeRate] = java.math.BigDecimal.valueOf(eurExchangeRate); it[this.eurExchangeDate] = LocalDate.parse(eurExchangeDate); it[this.amountEurCents] = amountEurCents
