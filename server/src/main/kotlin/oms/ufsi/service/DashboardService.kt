...
class DashboardService(private val auditLogService: AuditLogService) {
    fun get(allowedProjectIds: Set<Long>? = null) = transaction {
        ...
        val spent = FinancialRecordTable.selectAll().filter { it[FinancialRecordTable.projectId].value in projectIds && it[FinancialRecordTable.recordType] == "act" }.sumOf { it[FinancialRecordTable.amount] }
