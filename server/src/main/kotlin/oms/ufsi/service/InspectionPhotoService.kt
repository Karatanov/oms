...
class InspectionPhotoService(private val repository:InspectionPhotoRepository){
 ...
 fun upload(reportId:Long,name:String,contentType:String?,bytes:ByteArray):InspectionPhoto{require(repository.list(reportId).size<30){"An inspection report can contain no more than 30 photos."};require(bytes.size in 1..10_000_000){"Photo size must not exceed 10 MB."};val originalName=name.replace(Regex("[\\r\\n\\u0000]"),"").trim();require(originalName.isNotBlank()&&originalName.length<=255){"File name is invalid."};val ext=originalName.substringAfterLast('.',"").lowercase();require(ext in setOf("jpg","jpeg","png")){"Allowed photo formats: JPG, PNG."};require(matchesDeclaredFormat(bytes,ext)){"Photo content does not match its declared format."};val image=try{ImageIO.read(ByteArrayInputStream(bytes))}catch(_:Exception){null}?:throw IllegalArgumentException("Invalid image file.");require(image.width>0&&image.height>0){"Invalid image dimensions."};val uuid=UUID.randomUUID();val dir=uploadDirectory("inspection-photos");Files.createDirectories(dir);val original=dir.resolve("$uuid.$ext");val thumb=dir.resolve("$uuid-thumb.jpg");try{Files.write(original,bytes);val ratio=minOf(1.0,320.0/image.width);val width=(image.width*ratio).toInt();val height=(image.height*ratio).toInt();val target=BufferedImage(width,height,BufferedImage.TYPE_INT_RGB);val g=target.createGraphics();try{g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,RenderingHints.VALUE_INTERPOLATION_BILINEAR);g.drawImage(image,0,0,width,height,null)}finally{g.dispose()};ImageIO.write(target,"jpg",thumb.toFile());val photo=InspectionPhoto(0,uuid,reportId,originalName,original.toString(),thumb.toString(),contentType?:"image/$ext",bytes.size.toLong(),repository.list(reportId).isEmpty());repository.create(photo);return photo}catch(e:Exception){Files.deleteIfExists(original);Files.deleteIfExists(thumb);throw e}}
