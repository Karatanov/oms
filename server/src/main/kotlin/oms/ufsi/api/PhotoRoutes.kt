...
fun Route.photoRoutes(){route("/api/v1/inspection-reports/{reportUuid}/photos"){
 get{val r=call.photoReport()?:return@get;call.respond(AppContainer.inspectionPhotoService.list(r.id).map{it.toResponse()})}
 post{val r=call.photoReport()?:return@post;val m=call.receiveMultipart();var result:oms.ufsi.domain.InspectionPhoto?=null;try{m.forEachPart{p->if(p is PartData.FileItem&&p.name=="file"){val n=p.originalFileName?:throw IllegalArgumentException("File name is required.");result=AppContainer.inspectionPhotoService.upload(r.id,n,p.contentType?.toString(),p.provider().readRemaining(10_000_001).readByteArray())};p.dispose()};call.respond(HttpStatusCode.Created,(result?:throw IllegalArgumentException("Field file is required.")).toResponse())}catch(e:IllegalArgumentException){call.respond(HttpStatusCode.BadRequest,ErrorResponse("VALIDATION_ERROR",e.message?:"Invalid request."))}}
 put("{photoUuid}/main"){val r=call.photoReport()?:return@put;val p=call.parameters["photoUuid"]?.let{AppContainer.inspectionPhotoService.setMain(r.id,it)}?:return@put call.respond(HttpStatusCode.NotFound,ErrorResponse("NOT_FOUND","Photo not found."));call.respond(p.toResponse())}
 delete("{photoUuid}"){val r=call.photoReport()?:return@delete;val ok=call.parameters["photoUuid"]?.let{AppContainer.inspectionPhotoService.delete(r.id,it)}?:false;if(!ok)return@delete call.respond(HttpStatusCode.NotFound,ErrorResponse("NOT_FOUND","Photo not found."));call.respond(HttpStatusCode.NoContent)}
