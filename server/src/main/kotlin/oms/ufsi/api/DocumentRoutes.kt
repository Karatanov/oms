...
fun Route.documentRoutes() {
    route("/api/v1/projects/{projectUuid}/documents") {
        ...
        get("{documentUuid}/download") { val project=call.documentProject() ?: return@get; val doc=call.parameters["documentUuid"]?.let { AppContainer.projectDocumentService.get(project.id,it) } ?: return@get call.respond(HttpStatusCode.NotFound, ErrorResponse("NOT_FOUND","Document not found.")); val path=Path.of(doc.storagePath); if(!Files.isRegularFile(path)) return@get call.respond(HttpStatusCode.NotFound, ErrorResponse("NOT_FOUND","Document file is unavailable.")); val encodedName = URLEncoder.encode(doc.originalName, Charsets.UTF_8).replace("+", "%20"); call.response.header(HttpHeaders.ContentDisposition, "attachment; filename=\"download.pdf\"; filename*=UTF-8''$encodedName"); call.respondFile(path.toFile()) }
        delete("{documentUuid}") { call.requireRole("ADMIN", "PROJECT_MANAGER") ?: return@delete; val project=call.documentProject() ?: return@delete; val uuid=call.parameters["documentUuid"] ?: return@delete call.respond(HttpStatusCode.NotFound, ErrorResponse("NOT_FOUND","Document not found.")); if (!AppContainer.projectDocumentService.delete(project.id, uuid)) return@delete call.respond(HttpStatusCode.NotFound, ErrorResponse("NOT_FOUND","Document not found.")); call.respond(HttpStatusCode.NoContent) }
    }
}
