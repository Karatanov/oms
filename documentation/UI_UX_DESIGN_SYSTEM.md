...
Reports/documents: searchable registries, consistent table headers/actions, retry/empty states and local edit/move errors. Empty report and document registries return without resolving the project hierarchy; that hierarchy is loaded only when real rows require project context or when the user opens document upload. The report action group supports downloading or replacing its XLS/XLSX source file in place, which repairs missing ephemeral-host files without duplicating the report. Report author is the actual username returned by the server, not a fabricated administrator label. Manual SIR retains its XLS-aligned sections and localized health/safety prompts.
