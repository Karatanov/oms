...
def project_address(row: dict) -> str | None:
    name = str(row.get("I") or "").strip()
    settlement = str(row.get("M") or "").strip()
    oblast = str(row.get("C") or "").strip()
    match = re.search(r"(?:за адресою|по адресу)\s*[:\-]?\s*(.+)$", name, re.IGNORECASE)
    if match:
        return match.group(1).strip().rstrip(".")
    return ", ".join(part for part in (settlement, oblast) if part) or None
...
def build_coordinate_migration(rows: list[dict], geocoding: dict[str, dict]) -> str:
    lines = [
        "-- Replace coarse oblast-centre fallbacks with the best verified address/settlement coordinates.",
        "-- Generated by tools/generate_urp3_seed.py; source identifiers are stable across imports.",
    ]
    for row in rows:
        location = geocoding.get(str(row["F"]), {})
        if location.get("status") != "resolved":
            continue
        values = [
            project_address(row), location.get("latitude"), location.get("longitude"),
            location.get("matchLevel"), location.get("query"), location.get("displayName"), str(row["F"]),
        ]
        lines.append(
            "UPDATE projects p JOIN project_monitoring_details m ON m.project_id = p.id "
            "SET p.address = {0}, p.latitude = {1}, p.longitude = {2}, "
            "m.geocode_accuracy = {3}, m.geocode_query = {4}, m.geocode_display_name = {5} "
            "WHERE m.source_lot_id = {6};".format(*(sql(value) for value in values))
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    parser.add_argument("--coordinate-migration-output", type=Path)
...
"""
    ...
    if args.coordinate_migration_output:
        args.coordinate_migration_output.parent.mkdir(parents=True, exist_ok=True)
        args.coordinate_migration_output.write_text(build_coordinate_migration(rows, geocoding), encoding="utf-8")
